...
surrogates <- function(ts, surrogate.method, n.surrogates, print.to.console = F) {
    ...
    if (tolower(surrogate.method) == "aaft") {
        surr.data = replicate(n.surrogates, FractalPackageSurrogates(ts, method="aaft"))

        # Phase randomisation (Theiler, 1992).
    } else if (tolower(surrogate.method) == "phase") {
        surr.data = replicate(n.surrogates, FractalPackageSurrogates(ts, method="phase"))
        ...
        surr.data = replicate(n.surrogates, FractalPackageSurrogates(ts, method="ce"))
        ...
        surr.data = replicate(n.surrogates, FractalPackageSurrogates(ts, method="dh"))
...
FractalPackageSurrogates <- function(ts, method="aaft") {
    if (!method %in% c("aaft", "ce", "dh", "phase")) warning("ERROR: surrogate method is not valid")
    surr = fractal::surrogate(x=ts, method=method)
    return(surr)
}
