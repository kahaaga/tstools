...
summary_stats <- function(v,
                          percentiles = c(0.01, 0.05, 0.1, 0.3,
                                          0.7, 0.9, 0.95, 0.99),
                          as.df = F) {
    ...
    boxplotnotchlower <- v.median - (1.58 * v.IQR /
                                       sqrt(v.samplesize.without.nans))
    boxplotnotchupper <- v.median + (1.58 * v.IQR /
                                       sqrt(v.samplesize.without.nans))

    # Compute quantiles
    percentiles <- sapply(X = percentiles,  FUN = function(p) {
                            stats::quantile(v, p, na.rm = T)
                        })
