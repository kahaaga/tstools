...
basicstats <- function(v) {
    ...
    v.min = base::min(v, na.rm = T)
    v.max = base::max(v, na.rm = T)
    v.mean = base::mean(v, na.rm = T)
    v.median = stats::median(v, na.rm = T)
    v.mad = stats::mad(v, na.rm = T)
    v.IQR = stats::IQR(v, na.rm = T)
    v.p01 = stats::quantile(v, 0.01, na.rm = T)
    v.p05 = stats::quantile(v, 0.05, na.rm = T)
    v.p95 = stats::quantile(v, 0.95, na.rm = T)
    v.p99 = stats::quantile(v, 0.99, na.rm = T)
