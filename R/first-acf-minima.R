...
first_acf_minima <- function(v, lag.max, plot.acf = F) {
    ...
    # 99 percent confidence instead of 95 precent confidence.
    upper.uncertainty.bound <- stats::qnorm((1 + 0.99) / 2) / sqrt(201)
    if (length(which(autocorrelation.function < upper.uncertainty.bound)) > 0) {
        # Subtract 1 to accomodate for indexing in stats::acf
        lag <- which(autocorrelation.function < upper.uncertainty.bound)[1] - 1
    } else {
        # Subtract 1 to accomodate for indexing in stats::acf
        lag <- first_local_minima(autocorrelation.function) - 1
    }
